...
$ui = [];
try {
    $q = $pdo->prepare("SELECT key_group, key_name, value FROM translations WHERE lang = ? AND key_group IN ('vehicles','arac','second_hand','common','search','listing','map')");
    $q->execute([$lang]);
...
$label = static function (string $key, string $fallback = '') use ($ui): string {
    ...
    return (string)($ui['vehicles.' . $key] ?? $ui['common.' . $key] ?? ($alias && isset($ui[$alias]) ? $ui[$alias] : $fallback));
};
...
$fuelType = (string)($_GET['fuel_type'] ?? '');
$transmission = (string)($_GET['transmission'] ?? '');
$searchQuery = trim((string)($_GET['q'] ?? ''));
$sort = (string)($_GET['sort'] ?? 'newest');

// Build SQL query
$where = ["l.module = 'arac'", "l.status IN ('active','reserved')"];
$params = [];

if ($categoryId > 0 && $categoryId !== 4) {
    // Include this category and all its children (2 levels deep)
    $where[] = "(l.category_id = {$categoryId} OR l.category_id IN (SELECT id FROM categories WHERE parent_id = {$categoryId} OR parent_id IN (SELECT id FROM categories WHERE parent_id = {$categoryId})))";
}
...
if (in_array($fuelType, ['benzine', 'diesel', 'elektrisch', 'hybride', 'lpg'], true)) {
    $where[] = "EXISTS (SELECT 1 FROM listing_arac va WHERE va.listing_id = l.id AND va.fuel_type = :fuel_type)";
    $params['fuel_type'] = $fuelType;
}

if (in_array($transmission, ['automaat', 'handgeschakeld'], true)) {
    $where[] = "EXISTS (SELECT 1 FROM listing_arac vt WHERE vt.listing_id = l.id AND vt.transmission = :transmission)";
...
try {
    ...
    $catStmt = $pdo->prepare("SELECT c.id, c.parent_id, c.slug, c.icon, c.sort_order,
        ...
        (SELECT COUNT(*) FROM listings l WHERE l.module = 'arac' AND l.status IN ('active','reserved') AND (l.category_id = c.id OR l.category_id IN (SELECT id FROM categories WHERE parent_id = c.id))) AS listing_count
        FROM categories c
        ...
        WHERE c.module = 'arac' AND c.is_active = 1
    ...
    foreach ($allCats as $cat) {
        if (empty($cat['parent_id']) || (int)$cat['parent_id'] === 4) {
    ...
    // Do not mix positional and named placeholders. PDO rejects that query,
    // and the broad catch below made the page look empty instead of showing
    // the database error.
...
function getVehicleImage(array $item): string {
    $imgCandidates = array_filter([
        $item['image'] ?? null,
        $item['gallery_image'] ?? null,
        'listing_' . (int)$item['id'] . '_1.jpg',
        'img_' . (int)$item['id'] . '_1.jpg', 'listings/car-1.jpg'
    ]);
...
<script>
try {
    const vehicleScrollKey = 'avrupa_vehicle_scroll_y';
    const vehicleScroll = sessionStorage.getItem(vehicleScrollKey);
    sessionStorage.removeItem(vehicleScrollKey);
    window.addEventListener('DOMContentLoaded', function () {
        if (vehicleScroll !== null) {
            window.scrollTo(0, parseInt(vehicleScroll, 10) || 0);
        }
        document.documentElement.style.visibility = 'visible';
    }, { once: true });
    if (localStorage.getItem('avrupa_vehicle_view') === 'list') {
        document.documentElement.classList.add('sh-prefer-list');
    }
} catch (e) {}
</script>
...
<main class="sh-page">
    ...
    <div class="sh-container">
        <nav class="sh-breadcrumb" aria-label="Breadcrumb">
            ...
            <span class="sep">/</span>
            <a href="<?= e(BASE_URL) ?>/pages/search-vehicles.php" class="<?= !$categoryId ? 'current' : '' ?>"><?= e(t('nav.arac') ?: (t('vehicles.title') ?: 'Araçlar')) ?></a>
        ...
        <div class="sh-container">
            <div class="sh-hero-head">
                <div class="sh-hero-title">
                    <h1><?= e($selectedCategory ? $selectedCategory['name'] : (t('vehicles.title') ?: (t('nav.arac') ?: 'Araç İlanları'))) ?></h1>
            ...
            <div class="sh-cats-slider">
                <a href="<?= e(BASE_URL) ?>/pages/search-vehicles.php" class="sh-cat-card <?= !$categoryId ? 'active' : '' ?>">
                ...
                <?php foreach ($categoriesTree as $pCat): ?>
                    <a href="<?= e(BASE_URL) ?>/pages/search-vehicles.php?category_id=<?= (int)$pCat['id'] ?>" class="sh-cat-card <?= $categoryId === (int)$pCat['id'] ? 'active' : '' ?>">
    ...
    <div class="sh-container">
        <div class="sh-layout">
            ...
            <aside class="sh-sidebar">
                <form class="sh-filter-form" method="get" action="<?= e(BASE_URL) ?>/pages/search-vehicles.php" id="shFilterForm">
                    <?php if ($categoryId): ?>
                    ...
                    <div class="sh-filter-header">
                        ...
                        <a href="<?= e(BASE_URL) ?>/pages/search-vehicles.php" class="sh-clear-link"><?= e($label('clear_filters', 'Temizle')) ?></a>
                    </div>
                            ...
                            <?php foreach ($categoriesTree as $pCat): ?>
                                <div class="sh-cat-tree-item <?= ($categoryId === (int)$pCat['id'] || in_array($categoryId, array_column($pCat['children'], 'id'), true)) ? 'open active' : '' ?>">
                                    <div class="sh-cat-tree-header">
                                        <a href="<?= e(BASE_URL) ?>/pages/search-vehicles.php?category_id=<?= (int)$pCat['id'] ?>" class="sh-cat-tree-link <?= $categoryId === (int)$pCat['id'] ? 'current' : '' ?>">
                                    ...
                                    <?php if (!empty($pCat['children'])): ?>
                                        <div class="sh-cat-sub-list">
                                            <?php foreach ($pCat['children'] as $sCat): ?>
                                                <a href="<?= e(BASE_URL) ?>/pages/search-vehicles.php?category_id=<?= (int)$sCat['id'] ?>" class="sh-cat-sub-link <?= $categoryId === (int)$sCat['id'] ? 'current' : '' ?>">
                    ...
                    <!-- Fuel type -->
                    <div class="sh-filter-group">
                        <label class="sh-group-label"><?= e(t('vehicle.fuel') ?: 'Yakıt') ?></label>
                        <div class="sh-radio-options">
                            <label class="sh-radio-label">
                                <input type="radio" name="fuel_type" value="" <?= $fuelType === '' ? 'checked' : '' ?>>
                                <span><?= e(t('common.all') ?: 'Tümü') ?></span>
                            </label>
                            <label class="sh-radio-label">
                                <input type="radio" name="fuel_type" value="benzine" <?= $fuelType === 'benzine' ? 'checked' : '' ?>>
                                <span><?= e(t('vehicle.fuel_benzine') ?: 'Benzin') ?></span>
                            </label>
                            <label class="sh-radio-label">
                                <input type="radio" name="fuel_type" value="diesel" <?= $fuelType === 'diesel' ? 'checked' : '' ?>>
                                <span><?= e(t('vehicle.fuel_diesel') ?: 'Dizel') ?></span>
                            </label>
                            <label class="sh-radio-label">
                                <input type="radio" name="fuel_type" value="elektrisch" <?= $fuelType === 'elektrisch' ? 'checked' : '' ?>>
                                <span><?= e(t('vehicle.fuel_elektrisch') ?: 'Elektrikli') ?></span>
                            </label>
                        </div>
                    </div>
                    ...
                    <!-- Transmission -->
                    <div class="sh-filter-group">
                        <label class="sh-group-label"><?= e(t('vehicle.transmission') ?: 'Vites') ?></label>
                        <div class="sh-radio-options">
                            <label class="sh-radio-label">
                                <input type="radio" name="transmission" value="" <?= $transmission === '' ? 'checked' : '' ?>>
                                <span><?= e(t('common.all') ?: 'Tümü') ?></span>
                            </label>
                            <label class="sh-radio-label">
                                <input type="radio" name="transmission" value="automaat" <?= $transmission === 'automaat' ? 'checked' : '' ?>>
                                <span><?= e(t('vehicle.trans_automaat') ?: 'Otomatik') ?></span>
                            </label>
                            <label class="sh-radio-label">
                                <input type="radio" name="transmission" value="handgeschakeld" <?= $transmission === 'handgeschakeld' ? 'checked' : '' ?>>
                                <span><?= e(t('vehicle.trans_handgeschakeld') ?: 'Manuel') ?></span>
                            </label>
                        </div>
                    </div>
            ...
            <section class="sh-main">
                ...
                <div class="sh-toolbar">
                    <div class="sh-toolbar-info">
                        <h2><?= e($selectedCategory ? $selectedCategory['name'] : (t('nav.arac') ?: 'Tüm İkinci El İlanları')) ?></h2>
                        <span class="sh-total-badge"><?= number_format($total, 0, ',', '.') ?> <?= e(t('listing.listings_label') ?: 'ilan') ?></span>
                    </div>

                    <div class="sh-toolbar-controls">
                        <!-- Sort select -->
                        <form method="get" action="<?= e(BASE_URL) ?>/pages/search-vehicles.php" class="sh-sort-form">
                            ...
                            <article class="sh-card">
                                ...
                                <div class="sh-card-body">
                                    <div class="sh-card-header">
                                        ...
                                        <span class="sh-card-category"><?= e($item['category_name'] ?: 'Araçlar') ?></span>
                                    </div>
                ...
                <?php else: ?>
                    <div class="sh-empty-state">
                        ...
                        <a href="<?= e(BASE_URL) ?>/pages/search-vehicles.php" class="sh-reset-btn"><?= e($label('clear_filters', 'Filtreleri Sıfırla')) ?></a>
                    </div>
                <?php endif; ?>
            </section>
        </div>
...
document.addEventListener('DOMContentLoaded', function () {
    document.querySelectorAll('.sh-cat-card, .sh-cat-tree-link, .sh-cat-sub-link').forEach(function (link) {
        link.addEventListener('click', function () {
            try {
                sessionStorage.setItem('avrupa_vehicle_scroll_y', String(window.scrollY));
            } catch (e) {}
        });
    });
    ...
    if (gridBtn && listBtn && gridContainer) {
        const savedView = (() => {
            try { return localStorage.getItem('avrupa_vehicle_view'); } catch (e) { return null; }
        })();
        if (savedView === 'list') {
        ...
        gridBtn.addEventListener('click', function () {
            ...
            document.documentElement.classList.remove('sh-prefer-list');
            gridBtn.classList.add('active');
            listBtn.classList.remove('active');
            try { localStorage.setItem('avrupa_vehicle_view', 'grid'); } catch (e) {}
        });
        listBtn.addEventListener('click', function () {
            ...
            listBtn.classList.add('active');
            gridBtn.classList.remove('active');
            try { localStorage.setItem('avrupa_vehicle_view', 'list'); } catch (e) {}
        });
    }
});
</script>
